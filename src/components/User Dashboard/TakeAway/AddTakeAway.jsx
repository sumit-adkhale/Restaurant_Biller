import React, { useEffect, useState } from "react";
import Menu from "../Menu";
import { useDispatch, useSelector } from "react-redux";
import {
  addKOT,
  emptyTempCart,
  fetchFoodDetails,
  fetchTakeAwayDetails,
  removeTempItem,
  tempCart,
  updateOneTakeAwayDetails,
  updateQuantity,
  updateTakeAwayDetails,
  updateTempCart,
} from "../../../Store/Slice";
import { useNavigate } from "react-router-dom";
import AddOrderHeader from "../../Other Components/AddOrderHeader";
import Swal from "sweetalert2";
import { toast } from 'react-toastify';

function AddTakeAway({ id, action }) {
  const { tempCartDetails, takeAwayDetails, tempKOT } = useSelector(
    (state) => state.restaurant
  );

  const dispatch = useDispatch();
  const navigate = useNavigate();
  const [totalOrderValue, setTotalOrderValue] = useState(0);
  const [GST, setGST] = useState(0);
  const [grandTotal, setGrandTotal] = useState(0);
  const [payNow, setPayNow] = useState(false);
  const [newStringId, setNewStringId] = useState("");
  const [newSingleOrder, setNewSingleOrder] = useState("");

  const [input, setInput] = useState({
    id: "",
    customerName: "",
    customerContact: "",
    order: {},
    paymentDone: false,
    paymentMode: "",
  });

  useEffect(() => {
    if (!takeAwayDetails || takeAwayDetails.length === 0) {
      dispatch(fetchTakeAwayDetails());
    }
  }, [dispatch, takeAwayDetails]);

  useEffect(() => {
    dispatch(fetchFoodDetails());
    if (takeAwayDetails && takeAwayDetails.length > 0) {
      if (!action) {
        const allIds = takeAwayDetails.map((data) => Number(data.id)); // Ensure IDs are numbers
        const maxId = allIds.length > 0 ? Math.max(...allIds) : 0; // Handle empty array
        const newId = maxId + 1;

        setNewStringId(newId.toString());
      } else if (action === "EditTakeAway") {
        const singleTakeAway = takeAwayDetails.find((data) => data.id === id); // Use find instead of filter

        setNewSingleOrder(singleTakeAway);

        dispatch(updateTempCart(singleTakeAway.order));

        if (singleTakeAway) {
          setNewStringId(id);

          setInput({
            id: id.toString(),
            customerName: singleTakeAway.customerName || "",
            customerContact: singleTakeAway.customerContact || "",
            order: singleTakeAway.order || {},
            paymentDone: singleTakeAway.paymentDone || false,
            paymentMode: singleTakeAway.paymentMode || "",
          });
        }
      }
    }
  }, [dispatch, takeAwayDetails, id, action]);

  useEffect(() => {
    // Calculate total order value whenever tempCartDetails changes
    const total = tempCartDetails.reduce(
      (sum, item) => sum + item.price * (item.quantity || 1),
      0
    );

    setTotalOrderValue(total);
    const calculateGST = total * 0.05;
    setGST(calculateGST.toFixed(2));

    setGrandTotal(Math.floor(total + calculateGST));
  }, [tempCartDetails]);

  const getInputData = (e) => {
    setInput({ ...input, [e.target.name]: e.target.value });
  };

  const handleReset = () => {
    dispatch(emptyTempCart());
    setInput({
      id: "",
      customerName: "",
      customerContact: "",
      order: {},
      paymentDone: false,
      paymentMode: "",
    });
    setTotalOrderValue(0);
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (tempCartDetails.length === 0) {
      toast.warn("Please add some items to place order");
    } else {

      const getCurrentDate = () => {
        const date = new Date();
        const day = String(date.getDate()).padStart(2, "0"); // Ensure two digits
        const month = String(date.getMonth() + 1).padStart(2, "0"); // Months are 0-based
        const year = date.getFullYear();
        return `${day}/${month}/${year}`; // Format: DD/MM/YYYY
      };
      const currentTime=new Date().toLocaleTimeString()	
      const newOrder = {
        id: newStringId,
        customerName: input.customerName,
        customerContact: input.customerContact,
        order: tempCartDetails,
        orderValue: totalOrderValue,
        GST: GST,
        totalAmount: grandTotal,
        paymentDone: newSingleOrder.paymentDone || false,
        paymentMode: newSingleOrder.paymentMode || "",
        orderStatus: "Pending",
        PlacedDate:getCurrentDate(),
        PlacedTime:currentTime
      };
      const KOTdata = {
        id: newStringId,
        customerName: input.customerName,
        order: tempCartDetails,
      };

      if (!action) {
        dispatch(updateTakeAwayDetails(newOrder)); // Dispatch the new object
        dispatch(addKOT(KOTdata));
      } else if (action === "EditTakeAway") {
        const tempKOTData = {
          id: newStringId,
          customerName: input.customerName,
          order: tempKOT,
        };
        dispatch(updateOneTakeAwayDetails({ id: id, updatedData: newOrder }));
        dispatch(addKOT(tempKOTData));
      }
      if (payNow) {
        navigate(`/user/OrderSummery/${newStringId}`);
      } else {
        navigate("/user/take-away");
      }
      handleReset();
    }
    
  };
  return (
    <>
      <AddOrderHeader handleReset={handleReset} />
      <div className="py-5 relative top-0 left-0">
        <div className="flex justify-between items-center px-10 mb-4">
          <h1 className="pb-2">Order id (#) : {newStringId}</h1>

          <h1>
            <span className="text-md ">
              ₹ {totalOrderValue} (orde value) +{" "}
            </span>
            <span className="text-md ">₹ {GST} (5% GST) = </span>
            <span className="text-xl ">Grand Total : </span>
            <span className="text-2xl font-semibold">₹ {grandTotal}</span>
          </h1>
        </div>
        <div className="flex justify-between items-center px-10">
          <form className=" flex gap-4 pl-5" onSubmit={handleSubmit}>
            <input
              className=" placeholder-gray-700 pl-2 rounded-lg
                         border-gray-300 border-2 focus:outline-none
                          focus:border-blue-500"
              name="customerName"
              onChange={getInputData}
              type="text"
              value={input.customerName}
              placeholder="Customer Name"
              required
            />
            <input
              className=" placeholder-gray-700 px-4 rounded-lg
                         border-gray-300 border-2 focus:outline-none
                          focus:border-blue-500"
              name="customerContact"
              onChange={getInputData}
              value={input.customerContact}
              type="tel"
              placeholder="Customer Contact Number"
              maxLength={10}
              minLength={10}
            />
            <button className="bg-purple-600 cursor-pointer ml-7 text-xl px-3 rounded-md hover:bg-purple-700">
              Place Order
            </button>
            <button
              onClick={() => setPayNow(true)}
              className="bg-green-600 cursor-pointer mr-40 text-lg px-3 rounded-md hover:bg-green-700"
            >
              Place Order & Pay
            </button>
          </form>
          <button
            onClick={handleReset}
            className="bg-zinc-500 text-xl cursor-pointer px-3 rounded-md hover:bg-zinc-600"
          >
            Reset
          </button>
        </div>
        <div className="sticky top-0 left-0 mt-16 py-5 px-8 bg-zinc-800 rounded-xl shadow-md shadow-orange-300">
          <h1 className="text-2xl font-semibold text-center my-2">
            Added Food Menu
          </h1>
          <table className="w-full mt-2 text-center">
            <thead className="border-b-2 pb-2">
              <tr className=" font-semibold px-6 pr-16">
                <td className="px-6 py-2">Sr No.</td>
                <td>Food Items Name</td>
                <td>Veg / Non Veg</td>
                <td>cusine</td>
                <td>Price (Rs./₹)</td>
                <td>Quantity</td>
                <td>Action</td>
              </tr>
            </thead>
            <tbody className="gap-2 my-2">
              {tempCartDetails.length === 0 ? (
                <tr>
                  <td colSpan="7" className="text-2xl text-center">
                    Items Not Added !
                  </td>
                </tr>
              ) : (
                tempCartDetails.map((food, index) => (
                  <tr key={index} className="border-b-1 pb-2 px-8 mb-2">
                    <td className="px-6 py-2">{index + 1}</td>
                    <td>{food.foodName}</td>
                    <td>{food.type}</td>
                    <td>{food.cusine}</td>
                    <td className="pr-10">{food.price}</td>
                    <td>
                      <input
                        type="number"
                        value={food.quantity || 1}
                        min={1}
                        className="w-15 text-center"
                        onChange={(e) =>
                          dispatch(
                            updateQuantity({
                              id: food.id,
                              quantity: Number(e.target.value),
                            })
                          )
                        }
                      />
                    </td>
                    <td>
                      <button
                        onClick={() => dispatch(removeTempItem(food.id))}
                        className="bg-red-700 cursor-pointer mr-4 px-2 rounded"
                      >
                        Remove
                      </button>
                    </td>
                  </tr>
                ))
              )}
            </tbody>
          </table>
        </div>
      </div>
      <Menu newCol={true} />
    </>
  );
}

export default AddTakeAway;

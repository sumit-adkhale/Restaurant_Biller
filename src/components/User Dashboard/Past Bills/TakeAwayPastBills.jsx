...
import { fetchTakeAwayDetails } from "../../../Store/Slice";
import { useNavigate } from "react-router-dom";
import Loading from "../../Other Components/Loading";

function TakeAwayPastBills({admin}) {
    ...
    const {takeAwayDetails,takeAwayLoading}=useSelector((state) => state.restaurant)
    
  return (
    <>
      <div className="grid grid-cols-8 p-5 text-center">
        <div className="col-span-1 lg:col-span-1 max-sm:hidden">
          <p className="font-medium text-lg leading-8 ">Order Id (#)</p>
        </div>
        ...
        <div className="col-span-2  max-lg:hidden flex items-center justify-center">
          <p className="font-medium text-lg leading-8 ">Total Order Value</p>
        </div>
        <div className="col-span-1 max-lg:hidden">
          <p className="font-medium text-lg leading-8 ">Date & Time</p>
        </div>
        <div className="col-span-1 max-lg:hidden">
          <p className="font-medium text-lg leading-8 ">Status</p>
        </div>
        <div className="col-span-1 max-lg:hidden">
          <p className="font-medium text-lg leading-8 ">Bill Summery</p>
        </div>
      </div>
      {takeAwayLoading && <Loading/>}
      {takeAwayDetails.filter((data)=>data.orderStatus!=="Pending")?.slice().reverse().map((data,i)=>{
      ...
      return <div key={i} className="box p-2 text-center rounded-3xl bg-gray-100 grid grid-cols-8 mb-2  transition-all duration-500 hover:bg-indigo-50 max-lg:max-w-xl max-lg:mx-auto">
        ...
        <div className="col-span-8  sm:col-span-4 lg:col-span-2 flex h-full justify-center pl-4 flex-col max-lg:items-center">
        ...
        <div className="col-span-8 sm:col-span-4 lg:col-span-1 flex items-center justify-center">
          <p className="font-semibold text-xl leading-8 text-black">{data.PlacedDate}</p>
        </div>
        <div className="col-span-8 sm:col-span-4 lg:col-span-1 flex items-center justify-center ">
        ...
        <div className="col-span-8 sm:col-span-4 lg:col-span-1 flex items-center justify-center ">
          <button onClick={()=>navigate(`/${admin?"admin":"user"}/OrderSummery/${data.id}`,{state:{onlyViewBill:true}})} className="bg-amber-500 py-1 px-2 rounded-md cursor-pointer">
            View Bill
          </button>
        </div>
      </div>
...
export default TakeAwayPastBills;
